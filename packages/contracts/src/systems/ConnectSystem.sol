...
contract ConnectSystem is System {
  function connect(ConnectionType connectionType, bytes32 _organEntity) public {
    ...
    if (connectionType == ConnectionType.RESOURCE) {
      ...
      if (
        Type.get(_organEntity) == EntityType.FOOD_SOURCE
        || (Type.get(_organEntity) == EntityType.RESOURCE_SPLIT &&
          Type.get(ResourceConnection.get(_organEntity)) == EntityType.FOOD_SOURCE)
        || (Type.get(_organEntity) == EntityType.MODIFIER &&
          Type.get(ResourceConnection.get(_organEntity)) == EntityType.FOOD_SOURCE)
      ) {
  ...
  function disconnect(ConnectionType connectionType) public {
    ...
    if (connectionType == ConnectionType.RESOURCE) {
      if (
        Type.get(organEntity) == EntityType.FOOD_SOURCE 
        || Type.get(organEntity) == EntityType.RESOURCE_SPLIT
        || Type.get(organEntity) == EntityType.MODIFIER
      ) {
